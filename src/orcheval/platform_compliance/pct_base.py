...
PATCHED for qualitative error analysis robustness:
- Any exception inside a dimension computation is contained:
    -> dimension score = 0
    -> a CRITICAL issue is added with full traceback in details
    -> evaluation continues for remaining dimensions
- Gate checks are also guarded: if minimum_structure / import checks throw,
  gate fails with diagnostic details.
"""
...
def _tb_tail(max_chars: int = 8000) -> str:
    tb = traceback.format_exc()
    return tb if len(tb) <= max_chars else tb[-max_chars:]


def _exception_details(*, stage: str, exc: BaseException) -> Dict[str, Any]:
    return {
        "stage": stage,
...
class BasePlatformComplianceTester(BaseEvaluator):
    ...
    def evaluate(self, file_path: Path) -> EvaluationResult:
        ...
        # -------------------------
        # PATCH: contain exceptions per dimension, do not crash whole PCT.

        result.scores["loadability"] = self._safe_dimension_eval(
            dim="loadability",
            fn=lambda: self._evaluate_loadability(code, tree, file_path),
        )
        result.scores["structure_validity"] = self._safe_dimension_eval(
            dim="structure_validity",
            fn=lambda: self._evaluate_structure_validity(code, tree),
        )
        result.scores["configuration_validity"] = self._safe_dimension_eval(
            dim="configuration_validity",
            fn=lambda: self._evaluate_configuration_validity(code, tree),
        )
        result.scores["task_validity"] = self._safe_dimension_eval(
            dim="task_validity",
            fn=lambda: self._evaluate_task_validity(code, tree),
        )
        result.scores["executability"] = self._safe_dimension_eval(
            dim="executability",
            fn=lambda: self._evaluate_executability(code, tree, file_path),
        )
    ...
    # ---------------------------------------------------------------------
    # NEW: safe dimension wrapper
    # ---------------------------------------------------------------------

    def _safe_dimension_eval(self, *, dim: str, fn) -> EvaluationScore:
        """
        Execute a dimension evaluation function safely.
        If it throws, return score=0 with a CRITICAL issue + full traceback in details.
        """
        try:
            score: EvaluationScore = fn()
            # Ensure penalty-free invariant
            score.penalties_applied = 0.0
            if score.details is None:
                score.details = {}
            score.details.setdefault("_safe_dimension_eval", {"ok": True})
            return score

        except Exception as e:
            details = _exception_details(stage=f"dimension::{dim}", exc=e)
            issue = Issue(
                severity=Severity.CRITICAL,
                category="pct_exception",
                message=f"Exception while evaluating dimension '{dim}': {type(e).__name__}: {e}",
                details=details,
            )
    ...
    def _run_gate_checks(self, code: str, tree: ast.AST, file_path: Path) -> List[GateCheckResult]:
        ...
        try:
            ...
            gates.append(GateCheckResult(
                name="minimum_structure",
                passed=False,
                message=f"minimum_structure gate threw exception: {type(e).__name__}: {e}",
                is_critical=True,
                details=det,
            ))

        # Gate: critical imports sanity (PATCH: guard exceptions)
        try:
            ...
            det = _exception_details(stage="gate::critical_imports", exc=e)
            gates.append(GateCheckResult(
                name="critical_imports",
                passed=False,
                message=f"critical_imports gate threw exception: {type(e).__name__}: {e}",
                is_critical=True,
                details=det,
            ))
